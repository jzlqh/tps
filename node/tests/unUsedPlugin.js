...
const fs = require('fs');
const { searchFiles } = require('./unUsedUtils'); // deglob: Take a list of glob patterns and return an array of file locations（获取文件位置）
...
UnusedPlugin.prototype.apply = function apply(compiler) {
  const checkUnused = (compilation, callback) => {
    console.log('this.sourceDirectories', this.sourceDirectories);
